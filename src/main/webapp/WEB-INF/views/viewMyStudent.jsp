			        ...
			        <%-- Display account details if available --%>
						...
						<table class="table hover multiple-select-row data-table-export nowrap">
							...
							<tr>
								...
								<th class="text-left">Student Contact</th>
								<th class="text-left">Delete Student</th>
							</tr>
							</thead>
							<tbody>
							<%
				List<Student> list = (List<Student>) request.getAttribute("key1");
		    	
			   String lgName = (String)request.getAttribute("key2");
			   
		        for(Student Stu : list)
		        {
		        	String studLg = Stu.getStuLg();
		        	if(lgName.equals(studLg))
        			{
 	
    %> 
				  <tr>
						<td class="text-left"><%=Stu.getPRN() %></td>
						<td class="text-left"><%=Stu.getStuName() %></td>
						<td class="text-left"><%=Stu.getStuEmail() %></td>
					
						<td class="text-left"><%=Stu.getStuBranch() %></td>
						<td class="text-left"><%=Stu.getStuYear() %></td>
						<td class="text-left"><%=Stu.getStuDiv() %></td>
						<td class="text-left"><%=Stu.getStuCgpa() %></td>
						
						<td class="text-left"><%=Stu.getStuLg() %></td>
						<td class="text-left"><%=Stu.getStuMob() %></td>
						<td><a href="LGremoveStudent/<%=Stu.getPRN()%>">Delete</a></td>
